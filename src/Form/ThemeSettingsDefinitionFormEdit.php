 ...
 */
class ThemeSettingsDefinitionFormEdit extends FormBase {
   ...
   * {@inheritdoc}
   */
  public function buildForm(array $form, FormStateInterface $form_state, string $theme_settings = NULL, string $container_id = NULL, string $definition = NULL) {
    $this->themeSettings = \Drupal::entityTypeManager()->getStorage('theme_settings')->load($theme_settings);
    $this->definitionId = $definition;
    $configElement = $this->themeSettings->getDefinitionById($container_id, $this->definitionId);
    $manager = \Drupal::service('plugin.manager.pagedesigner_theme_settings.style_definition');
    $this->definitionPlugin = $manager->createInstance($configElement['plugin_id']);
    $this->definitionPlugin->themeSettings = $this->themeSettings;
    $this->definitionPlugin->getEditForm($form, $configElement);
    $form['actions'] = [
      '#type' => 'actions',
    ];
    $form['actions']['submit'] = [
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->themeSettings->setDefinitionById($this->definitionId, $values);
  }
